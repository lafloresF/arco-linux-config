...
complete -c dnsmasq -l filter-A -d 'Remove A records from answers.  No IPv4 addresses will be returned'
complete -c dnsmasq -l filter-AAAA -d 'Remove AAAA records from answers.  No IPv6 addresses will be returned'
...
complete -c dnsmasq -l nftset -d 'Similar to the --ipset option, but accepts one or more nftables  sets to add …'
...
complete -c dnsmasq -l strip-mac -d 'Remove any MAC address information already in downstream queries before forwa…'
...
complete -c dnsmasq -l strip-subnet -d 'Remove any subnet address already present in a downstream query before forwar…'
...
complete -c dnsmasq -l servers-file -d 'A special case of   --conf-file which differs in two respects'
complete -c dnsmasq -l conf-script -d 'Execute <file>, and treat what it emits to stdout as the contents of a config…'
